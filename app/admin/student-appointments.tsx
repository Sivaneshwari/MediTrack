...
export default function StudentAppointments() {
    ...
    const fetchAppointments = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${BASE_URL}/admin/dashboard/appointments/students`);
            if (res.ok) setAppointments(await res.json());
        } catch (error) { console.error(error); } finally { setLoading(false); }
    };

    const handleExport = (type: string) => {
        Linking.openURL(`${BASE_URL}/admin/dashboard/export/appointments/${type}`).catch(() => Alert.alert("Error", "Download failed"));
    };

    const handleShare = async () => {
        try {
            const summary = appointments.map(a => 
                `${a.full_name} (${a.student_id}) - ${a.status.toUpperCase()}\nReason: ${a.reason}\nNotes: ${a.nurse_notes || 'N/A'}`
            ).join('\n\n---\n\n');

            await Share.share({
                message: `Student Clinical Records Summary:\n\n${summary}`,
                title: 'Share Records'
            });
        } catch (error) {
    ...
    const toggleMedicine = (med: any) => {
        ...
        if (exists) {
            setSelectedMedicines(selectedMedicines.filter(m => m.id !== m.id));
        } else {
                ...
                <View style={styles.mobileCard}>
                    <View style={styles.mobileCardHeader}>
                        <View>
                            ...
                            <Text style={styles.tdSub}>{item.student_id}</Text>
                        </View>
    ...
    return (
        <AdminLayout title="Student Appointments" showSearch={false} showBackButton={true}>
            <View style={styles.topRow}>
                <View style={styles.headerRow}>
                    <View style={{ flex: 1 }}>
                        <Text style={styles.pageTitle}>Student Clinicals</Text>
