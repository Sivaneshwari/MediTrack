...
export default function StaffHome() {
    const { staff_id, name } = useLocalSearchParams();
    const staff_id_str = typeof staff_id === 'string' ? staff_id : staff_id?.[0] || '';
    const [healthTip, setHealthTip] = useState(HEALTH_TIPS[0]);
    ...
    const fetchData = useCallback(async () => {
        if (!staff_id_str) return;
        setLoading(true);
        try {
            const [apptRes, notifRes, prescRes, profileRes] = await Promise.all([
                fetch(`${BASE_URL}/staff/appointments/${staff_id_str}`),
                fetch(`${BASE_URL}/student/notifications/${staff_id_str}`),
                fetch(`${BASE_URL}/staff/prescriptions/${staff_id_str}`),
                fetch(`${BASE_URL}/staff/profile/${staff_id_str}`),
            ]);
    ...
    const handleUpdateProfile = async () => {
        setIsUpdating(true);
        try {
            const res = await fetch(`${BASE_URL}/staff/profile/${staff_id_str}`, {
                method: "PUT",
    ...
    const handleBookAppointment = async (data: any) => {
        try {
            const payload = {
                ...
                time: data.time,
                reason: data.reason,
                patient_id: data.student_id || staff_id_str,
                role: data.role || 'staff',
                priority: data.priority || 'Casual'
            };

            const res = await fetch(`${BASE_URL}/staff/appointment`, {
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        const msg = "Failed to book emergency.";
    ...
    return (
        <View style={styles.container}>
            ...
            <SafeAreaView style={{ flex: 1 }}>
                ...
                <View style={styles.header}>
                    <View style={styles.headerLeft}>
                        ...
                        <View style={{ marginLeft: 12 }}>
                            ...
                            <Text style={styles.headerName}>{name || 'Staff'}</Text>
                        </View>
                    </View>
                ...
                >
                    ...
                    <View style={styles.storyContainer}>
                        ...
                        <View style={styles.wellnessBox}>
                            ...
                            <Text style={styles.wellnessText}>{healthTip.text}</Text>
                        </View>
                    </View>

                    {/* Quick Stats / Actions (Exact grid from screenshot 2) */}
                    ...
                    <View style={styles.statsRow}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => router.push({ pathname: "/history", params: { userId: staff_id_str, userType: 'staff' } } as any)}
                        >
                    ...
                    <View style={styles.sheetOverlay}>
                        <View style={styles.sheetContainer}>
                            ...
                            <View style={styles.sheetHeader}>
                                ...
                                <Text style={styles.sheetName}>{profile?.full_name || name || 'Staff'}</Text>
                ...
                {/* Blood Notifications Modal */}
                <Modal visible={bloodHistoryModalVisible} transparent animationType="slide">
                    <View style={styles.sheetOverlay}>
                        <View style={[styles.sheetContainer, { maxHeight: '80%' }]}>
                            <View style={styles.sheetHandle} /><Text style={[styles.sheetName, { marginBottom: 20, textAlign: 'center' }]}>Blood Notifications</Text>
                            <ScrollView showsVerticalScrollIndicator={false}>
                                {notifications.filter((n: any) => n.type === 'blood_bank').length === 0 ? (
                                    <View style={[styles.emptyContainer, { borderWidth: 0 }]}><Ionicons name="notifications-off-outline" size={48} color={COLORS.muted} /><Text style={styles.emptyText}>No blood requests history.</Text></View>
                                ) : (
                                    notifications.filter((n: any) => n.type === 'blood_bank').map((n: any, i: number) => (
                                        <View key={i} style={[styles.cleanCard, { flexDirection: 'row', alignItems: 'center' }]}>
                                            <View style={[styles.calendarIcon, { backgroundColor: '#FEE2E2', width: 40, height: 40 }]}><Ionicons name="water" size={20} color={COLORS.danger} /></View>
                                            <View style={{ marginLeft: 12, flex: 1 }}>
                                                ...
                                                <Text style={[styles.apptTime, { marginTop: 4 }]}>{n.message}</Text>
                                            </View>
                                        </View>
                                    ))
                                )}
                            </ScrollView>
                            <TouchableOpacity style={[styles.outlineBtn, { marginTop: 20 }]} onPress={() => setBloodHistoryModalVisible(false)}><Text style={styles.outlineBtnText}>Close</Text></TouchableOpacity>
                        </View>
                    </View>
                </Modal>

                {/* Prescriptions Modal */}
                <Modal visible={prescriptionsModalVisible} transparent animationType="slide">
                    <View style={styles.sheetOverlay}>
                        <View style={[styles.sheetContainer, { maxHeight: '80%' }]}>
                            <View style={styles.sheetHandle} /><Text style={[styles.sheetName, { marginBottom: 20, textAlign: 'center' }]}>Active Prescriptions</Text>
                            <ScrollView showsVerticalScrollIndicator={false}>
                                {activePrescriptions.length === 0 ? (
                                    <View style={[styles.emptyContainer, { borderWidth: 0 }]}><Ionicons name="medkit-outline" size={48} color={COLORS.muted} /><Text style={styles.emptyText}>No active prescriptions.</Text></View>
                                ) : (
                                    activePrescriptions.map((group, idx) => (
                                        <View key={idx} style={styles.cleanCard}>
                                            <View style={styles.prescHeader}><Ionicons name="newspaper-outline" size={20} color={COLORS.text} /><Text style={styles.prescTitle}>{group.items.length} Medicines</Text></View>
                                            <View style={styles.divider} />
                                            {group.items.map((med: any, i: number) => (
                                                <View key={i} style={styles.medRow}><View style={styles.bullet} /><Text style={styles.medName}>{med.medicine_name}</Text><Text style={styles.medDose}>{med.dosage}</Text></View>
                                            ))}
                                        </View>
                                    ))
                                )}
                            </ScrollView>
                            <TouchableOpacity style={[styles.outlineBtn, { marginTop: 20 }]} onPress={() => setPrescriptionsModalVisible(false)}><Text style={styles.outlineBtnText}>Close</Text></TouchableOpacity>
                        </View>
                    </View>
                </Modal>

                {/* Daily Tip Modal (Exact same as student) */}
                <Modal visible={showTipModal} transparent animationType="fade">
                    <View style={styles.tipOverlay}>
                        <Animated.View style={[styles.tipCard, tipAnimatedStyle]}>
                            <View style={[styles.tipIllustration, { backgroundColor: healthTip.bg || '#F0F9FF' }]}>
                                <Ionicons name={healthTip.icon as any || "sparkles"} size={64} color={healthTip.color || COLORS.primary} />
                                <View style={[styles.blob, { backgroundColor: healthTip.color }]} />
                            </View>
                            <Text style={styles.tipTitle}>Daily Health Tip!</Text>
                            <Text style={styles.tipText}>{healthTip.text}</Text>
                            <TouchableOpacity style={styles.tipButton} onPress={closeTipModal}><Text style={styles.tipButtonText}>Thanks!</Text></TouchableOpacity>
                        </Animated.View>
                    </View>
                </Modal>
...
const styles = StyleSheet.create({
    ...
    storyInner: { flex: 1, backgroundColor: '#E3F2FD', borderRadius: 24, alignItems: 'center', justifyContent: 'center' },
    wellnessBox: { flex: 1 },
    ...
    tipOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 30 },
    tipCard: { width: '85%', backgroundColor: COLORS.white, borderRadius: 32, padding: 32, alignItems: 'center', shadowColor: "#000", shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 20, elevation: 10 },
    tipIllustration: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#EFF6FF', justifyContent: 'center', alignItems: 'center', marginBottom: 24, position: 'relative', overflow: 'hidden' },
    blob: { position: 'absolute', width: 120, height: 120, borderRadius: 60, opacity: 0.2, transform: [{ scale: 1.5 }] },
    tipTitle: { fontSize: 22, fontWeight: '800', color: COLORS.text, marginBottom: 8, textAlign: 'center' },
    tipText: { fontSize: 16, color: COLORS.textLight, textAlign: 'center', marginBottom: 24, lineHeight: 24, paddingHorizontal: 8 },
    tipButton: { backgroundColor: COLORS.primary, paddingVertical: 14, paddingHorizontal: 32, borderRadius: 16, shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
    tipButtonText: { fontSize: 16, fontWeight: '700', color: 'white' },
